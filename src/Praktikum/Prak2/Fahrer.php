...
//        }
    ...
    protected function generateView():void
    {
        ...
        $this->generatePageHeader('Fahrerseite'); //to do: set optional parameters
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($current_order_id != $data[$i]['ordering_id']) {
                if ($current_order_id !== NULL) { //Check if this is the first order
                    ...
                    echo "</form><p>Gesamtpreis: {$totalPrice} EUR</p>";
                }
            ...
            <form action="Fahrer.php" method="post">
                <p>{$data[$i]['name']} (Preis: {$data[$i]['price']} EUR)</p>               
        ...
        if ($current_order_id !== NULL) { //Display price of last order
            echo "<p>Gesamtpreis: {$totalPrice} EUR</p>";
        }
    ...
    protected function processReceivedData():void
    {
        ...
        if (isset($_POST['submit']) && isset($_POST['ordering_id']) && isset($_POST['status']) && isset($_POST['ordered_article_id'])) {
            $status = $_POST['status'];
